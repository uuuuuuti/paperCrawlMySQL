...
import logging,re,json,requests,urllib,random
...
class PatentSpider(CrawlSpider):
    ...
    def __init__(self, unit=None, *args, **kwargs):
        ...
        code =""
        for col in selected_collection:
            code = col['id']
    ...
    def parse(self, response):
        if response.status != 200:
            ...
            yield Request(patent_url,
                          meta={
                              'infoType':21,
                              'codeType':'inzl',
                              'catalogId':'lcatalog_inzl',
                              'catalogName':'申请的专利',
                              'code': code
                          },
    ...
    def parse_paper_url(self, response):
        if response.status != 200:
            ...
            for link in papers_link:
                ...
                yield Request(url=p_url, callback=self.parse_patentInfo)

    def parse_patentInfo(self, response):
        if response.status != 200:
            req = response.request
            req.meta["change_proxy"] = True
            yield req
            ...
            patent_id = re.findall(r'filename=\w*', response.url)[0].split('=')[-1]
            dbname = re.findall(r'dbname=\w*', response.url)[0].split('=')[-1]

            title = selector.xpath("/html/body/table[1]/tbody/tr/td[2]/text()").extract_first()
            authors = selector.xpath("").extract_first()
            parse_from = selector.xpath("").extract_first()
            unit = selector.xpath("").extract_first()
            abstract = selector.xpath("").extract_first()
            category = selector.xpath("").extract_first()
            main_clain = selector.xpath("").extract_first()
            pub_date = selector.xpath("").extract_first()
            apply_date = selector.xpath("").extract_first()
            url = selector.xpath("").extract_first()
